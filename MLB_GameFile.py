...
for A in Box:
    if os.path.isdir("Mlb_GameGame_Data\\"+A) == False:
        os.makedirs("Mlb_GameGame_Data\\"+A, exist_ok=True) #내용 없는 애들 날자 파일만 만들어주기
# Mlb_Game_Data 에서 gamePk 긁어오기


for A in Game_Day:
    if os.path.isdir("Mlb_GameGame_Data\\"+A) == False:
        os.makedirs("Mlb_GameGame_Data\\"+A, exist_ok=True)


#game file name 가져오기
dir = "Mlb_GameGame_Data"
file = os.listdir(dir)
for A in file:
    File_List.append(A[0:10])
#game file name 가져오기
...
for i in range(len(Game_Day)):
    ...
    for gameid in Game_ID[i]:
        if os.path.isfile("Mlb_GameGame_Data\\"+Game_Day[i]+"\\"+str(gameid)+".txt") == False:
            url = "https://baseball4.p.rapidapi.com/v1/mlb/games"
            querystring = {"gamePk":str(gameid)}
            headers = {
                "x-rapidapi-key": "비밀^^",
            ...
            with open("Mlb_GameGame_Data\\"+Game_Day[i]+"\\"+str(gameid)+".txt", "w", encoding="utf-8") as file:
                file.write(data)
